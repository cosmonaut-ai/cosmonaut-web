	...
	<div class="space-y-2">
		...
		<div class="relative">
			<Input
				...
				oninput={(e) => onConfirmPasswordChange((e.target as HTMLInputElement).value)}
				disabled={isSubmitting}
				autocomplete="new-password"
			/>
			<button
...
{#if isInAppBrowser}
	<p class="mt-2 text-center text-xs text-muted-foreground">
		...
		Google, or sign up with email.
	</p>
{/if}

<p class="mt-6 text-center text-sm text-muted-foreground">
	Already have an account?
	<button type="button" class="font-medium text-primary hover:underline" onclick={onSwitchToSignIn}>
		Sign in
	</button>
</p>
