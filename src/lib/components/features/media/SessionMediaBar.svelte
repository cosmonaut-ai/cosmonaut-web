<script lang="ts">
	...
	import { Play, Pause, Volume2, VolumeX, Check, EllipsisVertical, X } from '@lucide/svelte';
	...
	import SessionVolumeControls from './SessionVolumeControls.svelte';
...
>
	<div class="relative z-10 mx-auto flex max-w-4xl items-center gap-2 px-4 py-3 sm:gap-3">
		...
		{#if hasVolumeControls}
			<DropdownMenu.Root>
				<DropdownMenu.Trigger
					class="hidden shrink-0 rounded-md p-1.5 transition-colors sm:inline-flex {immersive
				...
				<DropdownMenu.Content
					...
					preventScroll={false}
				>
					<SessionVolumeControls {hasPlayableNarration} {hasSoundtrack} {immersive} />
				</DropdownMenu.Content>
			</DropdownMenu.Root>
		{/if}
		...
		<!-- Mobile overflow menu keeps secondary audio controls off the main bar. -->
		{#if hasPlayableNarration || hasSoundtrack}
			<DropdownMenu.Root>
				...
				>
					...
					{#if hasVolumeControls && hasPlayableNarration}
