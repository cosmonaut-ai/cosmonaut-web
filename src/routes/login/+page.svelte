					...
					{:else if view === 'signup'}
						<div class="space-y-4">
							...
							<div class="space-y-2">
								...
								{#if password.length > 0}
									<div class="grid grid-cols-2 gap-1 text-xs">
										<span
											class={passwordChecks.length ? 'text-green-500' : 'text-muted-foreground'}
										>
											{passwordChecks.length ? '\u2713' : '\u2022'} 8+ characters
										</span>
										<span
											class={passwordChecks.uppercase ? 'text-green-500' : 'text-muted-foreground'}
										>
											{passwordChecks.uppercase ? '\u2713' : '\u2022'} Uppercase
										</span>
										<span
											class={passwordChecks.lowercase ? 'text-green-500' : 'text-muted-foreground'}
										>
											{passwordChecks.lowercase ? '\u2713' : '\u2022'} Lowercase
										</span>
										<span
											class={passwordChecks.number ? 'text-green-500' : 'text-muted-foreground'}
										>
											{passwordChecks.number ? '\u2713' : '\u2022'} Number
										</span>
										<span
											class={passwordChecks.symbol ? 'text-green-500' : 'text-muted-foreground'}
										>
							...
							<Button
								...
								disabled={isSubmitting || !email || !passwordValid || !passwordsMatch}
							>
								{#if isSubmitting}
									<Spinner class="mr-2" />
								{/if}
